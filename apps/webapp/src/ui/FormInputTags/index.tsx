...
export const FormTagsInput = (props: FormTagsInputProps) => {
  return (
    <div class={props?.classWrapper ?? ''} {...props.api().rootProps}>
      <input
        placeholder={props?.placeholder ?? "Type your tag and press 'Enter'..."}
        class={textField({
      ...
      />
      <div class="flex flex-col md:flex-row flex-wrap gap-4 pt-2">
        <For each={props.api().value}>
          {(value, index) => (
            <span class="inline-flex w-[fit-content] border focus-within:ring-2 text-xs rounded bg-interactive-1 border-interactive-7 focus-within:border-interactive-10 py-0.5 md:py-0 px-2 text-interactive-11">
              <div class="flex items-center" {...props.api().getTagProps({ index: index(), value })}>
                <span class="block w-full h-full">{value} </span>
                <button {...props.api().getTagDeleteTriggerProps({ index: index(), value })}>
